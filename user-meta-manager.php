...
<br /> 
        ...
        <div class="umm-search-mode-menu hidden"> <select class="um-search-mode" name="umm_search_mode"><option value="ID"<?php if(!$_REQUEST['umm_search_mode'] || $_REQUEST['umm_search_mode'] == 'ID'): ?> selected="selected"<?php endif; ?>><?php _e('ID', 'user-meta-manager') ?></option><option value="user_login"<?php if($_REQUEST['umm_search_mode'] == 'user_login'): ?> selected="selected"<?php endif; ?>><?php _e('User Login', 'user-meta-manager') ?></option><option value="user_nicename"<?php if($_REQUEST['umm_search_mode'] == 'user_nicename'): ?> selected="selected"<?php endif; ?>><?php _e('Nice Name', 'user-meta-manager') ?></option><option value="display_name"<?php if($_REQUEST['umm_search_mode'] == 'display_name'): ?> selected="selected"<?php endif; ?>><?php _e('Display Name', 'user-meta-manager') ?></option><option value="user_registered"<?php if($_REQUEST['umm_search_mode'] == 'user_registered'): ?> selected="selected"<?php endif; ?>><?php _e('Date Registered', 'user-meta-manager') ?></option></select></div>
        <?php
...
<code>&copy;<a href="http://JasonLau.biz" target="_blank">JasonLau.biz</a></code> <code>[<?php _e($this->title . ' Version', 'user-meta-manager') ?>: <?php
$response = wp_remote_get('http://jasonlau.biz/user-meta-manager/version.run?version='.UMM_VERSION);
if(!is_wp_error($response)): echo $response['body']; else: echo UMM_VERSION; endif; ?>]</code>
...
function umm_load_scripts($hook) {
    if($hook && $hook == "users_page_user-meta-manager"):
       ...
       wp_register_script('umm_js', plugins_url('/js/user-meta-manager.js', __FILE__));
...
function editusermeta(){
    ...
    <input id="updateusermeta_submit" data-wait="'.__('Wait...', 'user-meta-manager').'" class="button-primary" type="submit" value="'.__('Update', 'user-meta-manager').'" />
...
function deleteusermeta(){
    ...
    <div class="updateusermeta-result hidden"></div>
    <input id="updateusermeta_submit" data-wait="'.__('Wait...', 'user-meta-manager').'" class="button-primary button-delete" type="submit" value="'.__('Delete', 'user-meta-manager').'" />
